...
#from  tf.sparse.SparseTensor import spmm
...
def MakeMLP(cfg: List[Union[str, int]], in_channels: int, out_channels: int) -> nn.Sequential:
    ...
    """
    layers: List[tf.nn.Module] = []
    for i in cfg:
        if isinstance(i, int):
            layers += [tf.nn.Linear(in_channels, i)]
            in_channels = i
        else:
            layers += [tf.nn.GELU()]
    layers += [tf.nn.Linear(in_channels, out_channels)]
    return tf.nn.Sequential(*layers)


class MLPBlock(tf.nn.Module):
    """
...
class AttentionModule(tf.nn.Module):
    def __init__(self,
                 embedding_size,
                 max_seq_len,
                 protocol,
        ...
        self.dropout1 = tf.nn.Dropout(self.dropout1_p)
        self.dropout2 = tf.nn.Dropout(self.dropout2_p)
    ...
    def forward(self, V, data):
        ...
        for m in range(self.n_W):
            ...
            if self.protocol == "dil":
                V = tf.sparse.sparse_dense_matmul(
...
class Paramixer(tf.nn.Module):
    def __init__(self,
        ...
        self.embedding =tf.nn.Embedding(
            self.vocab_size,
            self.embedding_size,
        ...
        # Init APC
        self.apc_embedding = tf.nn.Embedding(
            self.max_seq_len,
            self.embedding_size
        )

        self.attention = tf.nn.ModuleList(
            [
